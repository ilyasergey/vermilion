// NOTE: this file is a MODULE of the synthetic acquisition crate
// `layer_a.rs` (which supplies crate::specs::*, crate::lemmas::*, and the
// `subtle` extern rlib) — it does not compile standalone, so editor
// diagnostics on the bare file are expected; drive it through
// ./case-studies/dalek-lite/run.sh.
//
// <MODIFIED CODE> dalek-lite DL8 acquisition copy of
// upstream/curve25519-dalek/src/backend/serial/u64/field.rs (pin
// de9ebf015). The ONLY edits are the front-end mut-ref migration our
// newer pinned Verus requires: bare `self`/`*self`/`a`/`b` derefs in
// ENSURES clauses (which upstream's January pin read as final values)
// are wrapped in `final(..)`. Each edited clause is marked `// [final]`.
// Everything else is byte-identical; diff against the upstream file to
// audit. See docs/reports/dalek-lite-vstd-drift.md for the measured
// accommodation policy. </MODIFIED CODE>
// -*- mode: rust; -*-
//
...
impl<'a> AddAssign<&'a FieldElement51> for FieldElement51 {
    fn add_assign(&mut self, _rhs: &'a FieldElement51)
        ...
        ensures
            *final(self) == spec_add_fe51_limbs(old(self), _rhs),  // [final]
            fe51_as_nat(final(self)) == fe51_as_nat(old(self)) + fe51_as_nat(_rhs),  // [final]
            fe51_as_canonical_nat(final(self)) == field_add(  // [final]
...
impl<'a> SubAssign<&'a FieldElement51> for FieldElement51 {
    fn sub_assign(&mut self, _rhs: &'a FieldElement51)
        ...
        ensures
            fe51_limbs_bounded(final(self), 52),  // [final]
            *final(self) == spec_sub_limbs(old(self), _rhs),  // [final]
            fe51_as_canonical_nat(final(self)) == field_sub(  // [final]
                fe51_as_canonical_nat(old(self)),
                fe51_as_canonical_nat(_rhs),
            ),
    {
        ...
        */
        /* MODIFIED CODE */
        // [body accommodation] our newer pinned front end ICEs
        // (codegen_select_candidate) on the `&*self` reborrow-operator
        // selection; the explicit UFCS call is semantically identical.
        let result = core::ops::Sub::sub(&*self, _rhs);
...
impl<'a> MulAssign<&'a FieldElement51> for FieldElement51 {
    fn mul_assign(&mut self, _rhs: &'a FieldElement51)
        ...
        ensures
            fe51_as_canonical_nat(final(self)) == field_mul(  // [final]
                fe51_as_canonical_nat(old(self)),
                fe51_as_canonical_nat(_rhs),
            ),
            fe51_limbs_bounded(final(self), 54),  // [final]
    {
        // [body accommodation] see the Sub note above.
        let result = core::ops::Mul::mul(&*self, _rhs);
        self.limbs = result.limbs;
    }
}
...
impl ConditionallySelectable for FieldElement51 {
    ...
    // If choice is false, a and b remain unchanged
            ...
            !choice_is_true(choice) ==> final(a).limbs == old(a).limbs && final(b).limbs == old(b).limbs,  // [final]
            // If choice is true, a and b are swapped
            choice_is_true(choice) ==> final(a).limbs == old(b).limbs && final(b).limbs == old(a).limbs,  // [final]
    {
        // Originally this was
    ...
    // If choice is false, self remains unchanged
            ...
            !choice_is_true(choice) ==> final(self).limbs == old(self).limbs,  // [final]
            choice_is_true(choice) ==> final(self).limbs == other.limbs,  // [final]
            // Field element value preservation
            !choice_is_true(choice) ==> fe51_as_canonical_nat(final(self)) == fe51_as_canonical_nat(  // [final]
                old(self),
            ),
            choice_is_true(choice) ==> fe51_as_canonical_nat(final(self)) == fe51_as_canonical_nat(other),  // [final]
...
impl FieldElement51 {
    ...
    pub fn negate(&mut self)
        ...
        ensures
            ...
            u64_5_as_nat(final(self).limbs) == 16 * p() - u64_5_as_nat(old(self).limbs) - p() * ((  // [final]
            36028797018963952u64 - old(self).limbs[4]) as u64 >> 51),
            field_canonical(u64_5_as_nat(final(self).limbs) + u64_5_as_nat(old(self).limbs)) == 0,  // [final]
            final(self).limbs == spec_negate(old(self).limbs),  // [final]
    {
        proof {
            ...
            u8_32_as_nat(&r) == fe51_as_canonical_nat(&self),
    {
        ...
        // [cfg accommodation] upstream guards this runtime-only
        // debug_assert with #[cfg(not(verus_keep_ghost))] — excluded from
        // verification by upstream's own annotation; elided here (the
        // fork's debug_assert rewrite does not carry statement cfgs).
    ...
    {
        // [cfg accommodation] as above: runtime-only debug_assert,
        // upstream-excluded from verification; elided.

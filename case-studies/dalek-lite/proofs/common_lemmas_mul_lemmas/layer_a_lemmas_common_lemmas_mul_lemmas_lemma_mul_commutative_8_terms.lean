...
namespace layer_a.lemmas.common_lemmas.mul_lemmas.lemma_mul_commutative_8_terms

-- vrml:begin layer_a.lemmas.common_lemmas.mul_lemmas.lemma_mul_commutative_8_terms.ensures_8 00394f75e1bf21ff
/--
Generated VC `layer_a.lemmas.common_lemmas.mul_lemmas.lemma_mul_commutative_8_terms.ensures_8`.

- Rust source: `case-studies/dalek-lite/upstream/curve25519-dalek/src/lemmas/common_lemmas/mul_lemmas.rs:231:9-232:82`
- Rust/SST construct: function postcondition (`ensures`)
- AssertId: `8`
- Statement hash: `00394f75e1bf21ff`
-/
def ensures_8_meta : Vermilion.ObligationMeta := {
  ...
  endColumn := 82
  assertId := "8"
  functionName := "layer_a.lemmas.common_lemmas.mul_lemmas.lemma_mul_commutative_8_terms"
  kind := "ensures"
  statementHash := "00394f75e1bf21ff"
}
@[vrml_obligation] theorem ensures_8
    ...
    (a7 : Int)
    (b7 : Int)
    (call_lemma_mul_is_commutative_ensures_0 : (a0 * b0) = (b0 * a0))
    (call_lemma_mul_is_commutative_1_ensures_0 : (a1 * b1) = (b1 * a1))
    (call_lemma_mul_is_commutative_2_ensures_0 : (a2 * b2) = (b2 * a2))
    (call_lemma_mul_is_commutative_3_ensures_0 : (a3 * b3) = (b3 * a3))
    (call_lemma_mul_is_commutative_4_ensures_0 : (a4 * b4) = (b4 * a4))
    (call_lemma_mul_is_commutative_5_ensures_0 : (a5 * b5) = (b5 * a5))
    (call_lemma_mul_is_commutative_6_ensures_0 : (a6 * b6) = (b6 * a6))
    (call_lemma_mul_is_commutative_7_ensures_0 : (a7 * b7) = (b7 * a7)) :
    ((((((((a0 * b0) + (a1 * b1)) + (a2 * b2)) + (a3 * b3)) + (a4 * b4)) + (a5 * b5)) + (a6 * b6)) + (a7 * b7)) = ((((((((b0 * a0) + (b1 * a1)) + (b2 * a2)) + (b3 * a3)) + (b4 * a4)) + (b5 * a5)) + (b6 * a6)) + (b7 * a7)) := by
...
-- vrml:end layer_a.lemmas.common_lemmas.mul_lemmas.lemma_mul_commutative_8_terms.ensures_8

end layer_a.lemmas.common_lemmas.mul_lemmas.lemma_mul_commutative_8_terms

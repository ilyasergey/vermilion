...
-- vrml:spec mergesort_clean.is_sorted case-studies/merge-sort/mergesort_clean.rs:21:1
def mergesort_clean.is_sorted (v : (Vermilion.Seq Int)) : Prop :=
...
theorem is_sorted_push (r : Seq Int) (x : Int)
    (hr : mergesort_clean.is_sorted r)
    (hx : ∀ i : Int, 0 ≤ i → i < Seq.len r → Seq.index r i ≤ x) :
    mergesort_clean.is_sorted (Seq.push r x) := by
  intro i j hij
...
theorem is_sorted_add (r s : Seq Int)
    (hr : mergesort_clean.is_sorted r) (hs : mergesort_clean.is_sorted s)
    (hcross : ∀ i j : Int, 0 ≤ i → i < Seq.len r → 0 ≤ j → j < Seq.len s →
      Seq.index r i ≤ Seq.index s j) :
    mergesort_clean.is_sorted (Seq.add r s) := by
  intro i j hij
  obtain ⟨⟨hi0, hij'⟩, hj⟩ := hij
...
theorem is_sorted_subrange (v : Seq Int) (a b : Int)
    (hv : mergesort_clean.is_sorted v)
    (ha : 0 ≤ a) (hab : a ≤ b) (hb : b ≤ Seq.len v) :
    mergesort_clean.is_sorted (Seq.subrange v a b) := by
  intro i j hij
  obtain ⟨⟨hi0, hij'⟩, hj⟩ := hij

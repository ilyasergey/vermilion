// Fully verified merge sort on Vec<u64> (sortedness + multiset-permutation
// postconditions), VERBATIM from the Verus repository (examples/mergesort.rs,
// MIT/Apache-2.0) WITH all its inline proof scripting preserved — proof{}
// blocks, `broadcast use`, per-site lemma calls, `=~=` extensionality asserts.
// Vermilion verifies it by a combination of automated proof (the vrml ladder,
// broadcast-use hints, and assumed broadcast-lemma facts) and interactive Lean
// proofs in the proofs/ twin. The upstream `fn main` harness is omitted: its
// vec!/seq! macros pull in Box-allocation internals and `sorted_by` uses
// spec-closure comparators, both outside the sequential fragment. See
// mergesort_clean.rs for the same algorithm with the SMT scripting removed
// (specs only) — the interactive-proofs-first contrast.
use vstd::multiset::*;
use vstd::prelude::*;
